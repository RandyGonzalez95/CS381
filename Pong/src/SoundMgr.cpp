...
bool SoundMgr::initWatercraftSounds(){
        ...
        {
        	Entity381 * temp = engine->entityMgr->entities[i];

        	if(temp->audioId == 1)
        	{
        		this->registerSelection(*temp, selection2Filename);
        	}
        	else
        	{
        		this->registerSelection(*temp, selectionFilename);
        	}
        }
...
void SoundMgr::tick(float dtime)
{
    ...
    for(unsigned int i = 0; i < engine->entityMgr->entities.size(); i++)
    {
    	...
    	if(!temp->didSelectSoundPlay)
    	{
    		playSelectionSound(*temp);
    		temp->didSelectSoundPlay = true;
    	}
    	else if(temp->didSelectSoundPlay)
    	{
    		temp->didSelectSoundPlay = false;
    	}
    }
